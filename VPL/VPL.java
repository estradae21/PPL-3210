      ...
      // ********************************************
        ...
        if ( op == callCode) {
           ...
           numPassed = 0;
            ...
            ip = a; // Should be filled in during the loading of the program
        }
        else if ( op == condJumpCode) {
            // I think I found a massive bug in the design of VPL
            //ip = (getmem(b) != 0) ? a : ip;
            if (mem[bp + 2 + b] > 0) {
                ip = a;
            }
            ...
            //TODO
            setmem(a, (getmem(b) > 0| getmem(c) > 0)?1:0);
        }
        else if ( op == notCode) {
            ...
            setmem(a, mem[getmem(b) + getmem(c)]);
        }
        else if ( op == putCode) {
            mem[getmem(a) + getmem(b)] = getmem(c);
        }
        else if ( op == haltCode) {
            done = true;
        }
